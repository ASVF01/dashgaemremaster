...
export default function GameCanvas({ onHud, onFinish, onDeath, onInvboiPickup, paused, keepAudio = false, startAsInvboi = false, resetKey, levelId = "scribble-1" }: Props) {
  ...
  return (
    <div className="relative mx-auto" style={{ width: size.dw, height: size.dh }}>
      <canvas
        ...
        style={{ width: size.dw, height: size.dh, imageRendering: "pixelated" }}
      />
    </div>
  );
  ...
  function drawStarmanStars(ctx: CanvasRenderingContext2D, r: GameRefs) {
    ...
    for (let i = 0; i < SHIMMER_COUNT; i++) {
      ...
      const longR  = 7 * scale;
      const shortR = 1.6 * scale;
      ctx.beginPath();
      ctx.moveTo(0, -longR);
      ctx.lineTo(shortR, -shortR);
      ctx.lineTo(longR, 0);
      ctx.lineTo(shortR, shortR);
      ctx.lineTo(0, longR);
      ctx.lineTo(-shortR, shortR);
      ...
      ctx.beginPath();
      ctx.arc(-longR * 0.18, -longR * 0.18, Math.max(0.8, 1.1 * scale), 0, Math.PI * 2);
      ctx.fill();
      ctx.restore();
    }
  }
